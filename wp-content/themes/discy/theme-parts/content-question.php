...
if ($discoura_style == "on") {
	...
	$privacy_credential = (has_wpqa()?wpqa_check_user_privacy($post_data->post_author,"credential"):"");
	if (!is_single()) {
		$question_simple = "on";
	}
...
if (!$is_super_admin && $yes_private != 1 && $private_question_content != "on") {
							...
							<div>
								<?php if (is_single()) {
									...
									if ($breadcrumbs != "on" || ($breadcrumbs == "on" && ($breadcrumbs_style != "style_2" || ($breadcrumbs_content_title != "on" && $breadcrumbs_style == "style_2")))) {
										the_title( '<'.($breadcrumbs == "on" && $breadcrumbs_style == "style_2"?"h2":"h1").' class="'.(isset($title_post_style) && $title_post_style == "style_2"?"post-title-2":"post-title").'">'.(isset($title_post_style) && $title_post_style == "style_2" && isset($title_post_icon) && $title_post_icon != ""?"<i class='".$title_post_icon."'></i>":"").'<span'.(is_single()?' itemprop="name"':'').'>', apply_filters("discy_filter_after_title_question",false).'</span></'.($breadcrumbs == "on" && $breadcrumbs_style == "style_2"?"h2":"h1").'>' );
									}
								}else {
									if ($pending_questions) {
								...
								<div class="question-content-text">
									...
									if (($pending_questions || is_single() || (!is_single() && $question_poll_loop == "on")) && $question_poll == "on") {
										...
										<div class='all_single_post_content poll-area<?php echo ($pending_questions?" discy_hide":"")?>'>
											<?php echo (has_wpqa()?wpqa_show_poll_results($post_data->ID,$user_id):"")?>
										</div><!-- End poll-area -->
									<?php }
							...
							<footer class="question-footer<?php echo ($pending_questions?" pending-post-footer":"").($question_meta_icon == "on"?" question-footer-icons":"")?>">
								<?php if (has_wpqa() && $pending_questions) {
									...
									<ul class="footer-meta">
										...
										if (has_wpqa() && is_single() && ($share_style == "style_2" || $question_simple == "on") && ($share_facebook == "share_facebook" || $share_twitter == "share_twitter" || $share_linkedin == "share_linkedin" || $share_whatsapp == "share_whatsapp")) {?>
	...
	<?php }else {
		echo '<article class="article-question private-question article-post clearfix">
			<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have a permission to view questions.","discy").'</p></div>
