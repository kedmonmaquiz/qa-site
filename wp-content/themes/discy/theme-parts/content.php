...
if ('post' !== $post_data->post_type || ('post' === $post_data->post_type && ($custom_permission != "on" || $is_super_admin || (is_user_logged_in() && isset($roles["show_post"]) && $roles["show_post"] == 1) || (!is_user_logged_in() && $show_post == "on") || ($user_id > 0 && $user_id == $post_data->post_author)))) {
	...
	echo '<article class="private-post article-post clearfix">
		<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have a permission to view posts.","discy").'</p></div>
