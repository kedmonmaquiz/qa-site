...
$comments_args = array(
	'must_log_in'          => '<div class="alert-message warning"><i class="icon-flag"></i><p>'.($post->post_type == 'question'?esc_html__("You must login to add an answer.","discy"):esc_html__("You must login to add a new comment.","discy")).'</p></div>'.do_shortcode("[wpqa_login]".'<div class="pop-footer pop-footer-comments">'.wpqa_signup_in_popup().'</div></div>'),
