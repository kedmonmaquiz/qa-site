		...
		<div class="post-inner">
			...
			if ($filter_show_comments == true) {
				if ($post->post_type == 'question') {
					...
					if (isset($answers_tabs_foreach) && $answers_tabs_foreach == true && $post->post_type == 'question' && isset($first_one) && $first_one !== "") {?>
				...
				<?php if (isset($answers_tabs_foreach) && $answers_tabs_foreach == true && $post->post_type == 'question' && isset($first_one) && $first_one !== "") {?>
				...
				if (($post->post_type != 'question' && ($is_super_admin || $custom_permission != "on" || (is_user_logged_in() && $custom_permission == "on" && isset($roles["show_comment"]) && $roles["show_comment"] == 1) || (!is_user_logged_in() && $show_comment == "on"))) || (($post->post_type == 'question') && ($is_super_admin || $custom_permission != "on" || (is_user_logged_in() && $custom_permission == "on" && isset($roles["show_answer"]) && $roles["show_answer"] == 1) || (!is_user_logged_in() && $show_answer == "on")))) {
					if ($post->post_type == 'question') {
						if (isset($first_one) && $first_one !== "") {
							if ($user_id > 0) {
				...
				<?php }else {
					if ($post->post_type == 'question') {
						echo '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have a permission to view answers.","discy").' '.(has_wpqa()?wpqa_paid_subscriptions():'').'</p></div>';
					}else {
						echo '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have a permission to view comments.","discy").' '.(has_wpqa()?wpqa_paid_subscriptions():'').'</p></div>';
					}
				}
			}?>
			<div class="clearfix"></div>
