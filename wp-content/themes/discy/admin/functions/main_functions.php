...
define("discy_theme_version","4.5.1");
	...
	function discy_enqueue_scripts( $hook ) {
		...
		if ( $enqueue ) {
			...
			$discy_js = array(
				...
				"show_question"             => esc_html__("Select ON to allow users to view questions.","discy"),
				...
				"show_answer"               => esc_html__("Select ON to allow users to view answers.","discy"),
				...
				"show_post"                 => esc_html__("Select ON to allow users to view posts.","discy"),
				"add_comment"               => esc_html__("Select ON to allow users to add a comment.","discy"),
				"show_comment"              => esc_html__("Select ON to allow users to view comments.","discy"),
			);
...
function discy_comment($comment,$args,$depth,$answer = "",$owner = "",$k_ad = "",$best_answer = "",$answer_args = array()) {
	...
	if ($like_answer == true || ($answer != "answer" && isset($post_data->post_type) && $post_data->post_type == 'question') || ($answer == "answer" && $post_type == "question")) {
		$yes_private_answer = (has_wpqa()?wpqa_private_answer($comment_id,$comment_user_id,$user_get_current_user_id,$post_data->post_author):1);
	...
	$privacy_credential = (has_wpqa()?($comment_user_id > 0?wpqa_check_user_privacy($comment_user_id,"credential"):""):"");
	...
	if ($yes_private_answer != 1) {?>
		<li class="comment byuser comment">
			<div class="comment-body clearfix">
				<div class="comment-text"><?php esc_html_e("Sorry, this is a private answer.","discy");?></div>
			</div>
	<?php }else {?>
			...
			<div id="comment-<?php echo esc_attr($comment_id);?>" class="comment-body clearfix">
				...
				<div class="comment-text">
					...
					<div class="author clearfix">
						...
						<div class="comment-meta">
							<div class="comment-author">
								<?php $activate_comment_author = apply_filters('discy_activate_comment_author',true,$comment_id);
								if ($activate_comment_author == true) {
					...
					<div class="text">
						...
						if ($post_type == "posts") {
							$featured_image_in_answers = discy_options("featured_image_group_post_comments");
							$featured_image_answers_lightbox = discy_options("featured_image_group_post_comments_lightbox");
							$featured_image_answer_width = discy_options("featured_image_group_post_comments_width");
							$featured_image_answer_height = discy_options("featured_image_group_post_comments_height");
						...
						<div<?php echo (isset($its_question) && $its_question == "question" && is_single()?" itemprop='text'":"")?>>
							<?php if (isset($answer_args["comment_with_title"]) || isset($answer_args["comment_read_more"]) || isset($args["comment_read_more"])) {
								...
								echo '<p class="less_answer_text'.($strlen_comment < $comment_excerpt_count?" wpqa_hide":"").'">'.wp_html_excerpt($comment->comment_content,$comment_excerpt_count,'<a class="post-read-more comment-read-more read_more_answer" href="'.$comment_link.'" rel="bookmark" title="'.esc_attr__('Read more','discy').'">'.esc_html__('Read more','discy').'</a>').'</p>
						...
						<ul class="comment-reply comment-reply-main">
							<?php if (($answer != "answer" && $answer != "comment") || (isset($args["comment_type"]) && $args["comment_type"] == "comment_group")) {
								if ($answer != "posts") {
									...
									$add_comment = discy_options("add_comment");
							...
							if (isset($answer_args["show_replies"]) && $answer_args["show_replies"] == true || !isset($answer_args["show_replies"])) {
								do_action("discy_action_show_replies",$comment,(isset($its_question) && $its_question == "question"?"answer":"comment"));
							}
	...
	if ($custom_permission == "on") {
		...
		if (($custom_permission == "on" && is_user_logged_in() && !is_super_admin($user_id) && empty($roles["without_ads"])) || ($custom_permission == "on" && !is_user_logged_in())) {
			if ($class == "discy-ad-header") {
				...
				if (has_wpqa() && $subscriptions_payment == 'on') {
			...
			foreach ($discy_home_tabs as $key => $value) {
				if ((isset($discy_home_tabs[$key]["sort"]) && isset($discy_home_tabs[$key]["value"]) && $discy_home_tabs[$key]["value"] != "" && $discy_home_tabs[$key]["value"] != "0") || (isset($discy_home_tabs[$key]["value"]) && isset($discy_home_tabs[$key]["cat"]))) {
					if (isset($discy_home_tabs[$key]["value"]) && isset($discy_home_tabs[$key]["cat"])) {
						if (is_numeric($discy_home_tabs[$key]["value"]) && $discy_home_tabs[$key]["value"] > 0) {
			...
			foreach ($last_url as $last_key => $last_value) {
				...
				<?php }
						if (isset($last_value["link"]) && (isset($discy_home_tabs[$last_value["link"]]["sort"]) || (isset($last_value["link-2"]) && isset($discy_home_tabs[$last_value["link-2"]]["sort"])))) {
							...
							if (is_numeric($discy_home_tabs[$last_key]["value"]) && $discy_home_tabs[$last_key]["value"] > 0) {
