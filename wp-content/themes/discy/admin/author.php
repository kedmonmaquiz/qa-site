...
function discy_admin_author($user_id = "") {
	...
	if (is_super_admin($get_current_user_id)) {
		...
		$html_content = '<a class="button fixed-counts" data-user="'.$user_id.'" href="'.admin_url("user-edit.php?user_id=".$user_id."&rows=fixed").'">'.esc_html__("Fixed all the user counts","discy").'</a>';
		$options[] = array(
			'name' => $html_content,
			'type' => 'info'
		);
