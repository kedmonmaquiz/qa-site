...
function discy_options_uploader ($_id,$_value,$_desc = '',$_name = '',$no_id_name = '',$options = array()) {
	...
	if ($value_id == 0 && $value != "") {
		if (strpos($value,esc_url(home_url('/'))) !== false && strpos($value,"themes/".get_template()."/image") === false) {
			$value_id = discy_get_attachment_id($value);
		}
	}
...
/* Get fields */
function discy_options_fields($settings = array(),$option_name = "",$page = "options",$post_term = 0,$options_arrgs = array()) {
	global $allowedtags;
	$wp_page_template = ($page == "meta" && isset($post_term) && $post_term > 0?discy_post_meta("_wp_page_template",$post_term,false):"");
	if ($option_name == "") {
			...
			if ($value['type'] != "heading-2" && $value['type'] != "heading-3" && $value['type'] != "content" && $value['type'] != "info" && $value['type'] != 'hidden') {
				...
				if ($value['type'] != "heading" && $value['type'] != "heading-2" && $value['type'] != "heading-3" && $value['type'] != "info" && $value['type'] != "content" && $value['type'] != 'hidden') {
					if ($value['type'] == "checkbox") {
						...
						$output .= '<div class="explain discy_help"><div>'.wp_kses($explain_value,$allowedtags).'</div></div>';
					}
				}
