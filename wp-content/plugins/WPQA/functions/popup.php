	...
	function wpqa_head_content($login = "",$its_not_login = false) {
		...
		if ($confirm_email != "yes" && $login != "login") {
			...
			if ($active_message == "on" && (wpqa_is_user_profile() || $active_message_filter == true)) {
				...
				if (wpqa_is_user_messages() || ((!is_user_logged_in() && $send_message_no_register == "on") || (is_user_logged_in() && (empty($user_block_message) || (isset($user_block_message) && is_array($user_block_message) && !in_array($user_id,$user_block_message))) && ($block_message != "on" || is_super_admin($wpqa_user_id)) && isset($received_message) && $received_message == "on"))) {
					$custom_permission = wpqa_options("custom_permission");
