		...
		if (isset($group_users_array) && is_array($group_users_array) && !empty($group_users_array)) {
			echo "<div id='section-group-users' class='user-section row user-not-normal user-section-columns'>";
				...
				foreach ($get_results as $user) {
					echo "<div class='col ".$user_col."'>".wpqa_author($user->ID,"columns","","","","","","","","",$group_id,"block",(isset($blocked_users_array) && is_array($blocked_users_array) && !empty($blocked_users_array)?$blocked_users_array:array()),(isset($group_moderators) && is_array($group_moderators) && !empty($group_moderators)?$group_moderators:array()))."</div>";
				}
			echo "</div>";
